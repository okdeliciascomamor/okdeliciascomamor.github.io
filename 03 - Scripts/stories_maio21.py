"""
3 Stories Prova Social — Ok Delícias com Amor — 21/05/2026
Conceito: o salgado que ninguém esquece depois da festa.
Arc: gancho > produto > CTA
Padrões: Lora-Bold para títulos, outline em script, acentuação correta.
"""
...
def script_outlined_wrap(draw, text, font, max_w, cx, y,
    ...
    """Script com outline escuro e word-wrap. Retorna y após última linha."""
    if fill is None:
        fill = OURO_P
    words = text.split()
    lines = []
...
def c_text_shadow(draw, text, font, cx, y, fill, shd=4, shd_col=(4, 2, 0)):
    """Texto centralizado com sombra. Retorna y + altura."""
...
# ── STORY 1: GANCHO — prova social ──────────────────────────────────────────
# "tem um salgado que ninguém esquece depois da festa."
# Visual: foto escura (IMG_1059), texto impactante
def story_1():
    img  = foto_fundo(os.path.join(BASE_DIR, "fotos", "IMG_1059.JPEG"),
    ...
    # Script com outline — centralizado verticalmente no canvas
    sc_y = H // 2 - 300
    sc_end = script_outlined_wrap(draw, "você já provou?",
                                   fnt_sc, W-130, cx, sc_y,
                                   fill=OURO_P, line_sp=6)

    draw.line([(cx-80, sc_end+14), (cx+80, sc_end+14)], fill=OURO_S, width=2)

    # Título — wrap garante que não sai da borda
    ty = sc_end + 44
    ty = wrap_centered(draw, "tem um salgado que ninguém esquece.",
                        fnt_tit, W-100, cx, ty,
                        fill=CREME, shadow=True, line_spacing=10)
    ty += 34

    # Divisor
    draw.line([(cx-60, ty), (cx+60, ty)], fill=OURO_S, width=1)
    ty += 30

    # Body
    ty = wrap_centered(draw, "é o que os convidados da Oliete sentem.",
                        fnt_body, W-120, cx, ty,
                        fill=(220, 202, 178), shadow=True, line_spacing=8)
    ty += 40

    # Detalhe
    det_b = draw.textbbox((0, 0), "sempre pedem de volta.", font=fnt_det)
    draw.text((cx-(det_b[2]-det_b[0])//2+3, ty+3),
              "sempre pedem de volta.", font=fnt_det, fill=(4, 2, 0))
    draw.text((cx-(det_b[2]-det_b[0])//2, ty),
              "sempre pedem de volta.", font=fnt_det, fill=OURO_P)
    ...
    out = os.path.join(BASE_DIR, "story_maio21_1.png")
    img.convert("RGB").save(out, "PNG", quality=98)
    print(f"Salvo: {out}")


# ── STORY 2: PRODUTO — o artesanal de verdade ───────────────────────────────
# Visual: fundo claro, foto em arco (IMG_1108), editorial premium
def story_2():
    ...
    foto = Image.open(os.path.join(BASE_DIR, "fotos", "IMG_1108.JPEG")).convert("RGB")
    ...
    # Título — duas linhas explícitas, sem risco de quebra estranha
    tl1_b = draw.textbbox((0, 0), "cada salgado", fnt_tit)
    draw.text((cx-(tl1_b[2]-tl1_b[0])//2, base_y), "cada salgado", font=fnt_tit, fill=TXT)
    base_y += (tl1_b[3]-tl1_b[1]) + 6
    tl2_b = draw.textbbox((0, 0), "conta uma história.", fnt_tit)
    draw.text((cx-(tl2_b[2]-tl2_b[0])//2, base_y), "conta uma história.", font=fnt_tit, fill=OURO)
    base_y += (tl2_b[3]-tl2_b[1]) + 34

    draw.line([(cx-60, base_y), (cx+60, base_y)], fill=OURO_P, width=2)
    base_y += 28

    # Body
    base_y = wrap_centered(draw, "artesanal, afetivo, de verdade.",
                            fnt_body, W-120, cx, base_y,
                            fill=TXT_M, shadow=False, line_spacing=8)
    base_y += 20

    # Detalhe
    det_b = draw.textbbox((0, 0), "encomendas para junho e julho", font=fnt_det)
    draw.text((cx-(det_b[2]-det_b[0])//2, base_y),
              "encomendas para junho e julho", font=fnt_det, fill=TXT_M)
    ...
    border(draw)
    out = os.path.join(BASE_DIR, "story_maio21_2.png")
    img.convert("RGB").save(out, "PNG", quality=98)
    print(f"Salvo: {out}")


# ── STORY 3: CTA ────────────────────────────────────────────────────────────
# "garanta sua data. junho enchendo rápido."
# Visual: foto escura (IMG_1130), chamada direta, botão dourado
def story_3():
    img  = foto_fundo(os.path.join(BASE_DIR, "fotos", "IMG_1130.JPEG"),
    ...
    cy = script_outlined_wrap(draw, "o seu evento merece isso",
                               fnt_sc, W-130, cx, cy,
    ...
    # "garanta" — Lora-Bold 138px
    cy = c_text_shadow(draw, "garanta", fnt_tit, cx, cy, fill=CREME, shd=5) + 6

    # "sua data." — dourado
    cy = c_text_shadow(draw, "sua data.", fnt_tit, cx, cy, fill=OURO_P, shd=5) + 36

    draw.line([(cx-72, cy), (cx+72, cy)], fill=(100, 78, 40), width=2)
    cy += 28

    # Body
    cy = wrap_centered(draw, "junho enchendo rápido.",
                        fnt_body, W-140, cx, cy,
                        fill=(205, 185, 155), line_spacing=10, shadow=True)
    cy += 40

    # Botão CTA dourado
    det   = "fala com a Oliete agora"
    ...
    out = os.path.join(BASE_DIR, "story_maio21_3.png")
    img.convert("RGB").save(out, "PNG", quality=98)
    print(f"Salvo: {out}")
